...
ccccccc
            do k=1,n_int
                  write(21,*) psi(:,k)    !basis在fort.21里面
            end do
!积分算矩阵元
            do i=0,n_basis
                  do j=0,n_basis                 
     ...
     &               +(hbarc*hbarc/2/mu)*(l+1d0)*l/rr(k)**2*rrw(k)    !离心位垒项
                  end do
                  end do
            end do
...
ccccccc
      ...
      !      do u=0,n_basis
            do i=1,n_int
                  s=0
                  do j=0,n_basis
                        s=s+vr(j,ll)*psi(j,i)    !第二个指标u对应了wr的index,即第几列特征向量
                  end do
...
ccccccc
            ...
            write(*,*)'n_match=',n_match
            write(*,*)'r_match=',rr(n_match)
ccccccc           
